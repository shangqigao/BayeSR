...
class Tester(BayeSR):
    ...
    def run_test(self):
        test_lr_dir = {'DEN': '{}/{}'.format(args.input_data_dir, args.dataset),
                       ...
                       'SySR': '{}/{}/LR_degraded/X{}'.format(args.input_data_dir, args.dataset, args.upscale),
        ...
        test_sr_dir = '{}/{}_BayeSR_{}_{}_x{}_read{}_shot{}'.format(args.save_dir, args.dataset, img_mode, args.task, args.upscale, args.sigma_read, args.sigma_shot)
        ...
        for number in samples:
            with tf.Graph().as_default():
                ...
                gt_path = os.path.join(test_gt_dir, gt_names[number]) if test_gt_dir is not None else None
                ...
                if args.ensemble:
                    ...
                    output0 = self.inference(image_pl0, is_training=False)
                    output1 = self.inference(image_pl1, is_training=False)
                    bayesr_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, 'SmSpNet')
                    bayesr_vars = [v for v in bayesr_vars if 'Discriminator' not in v.name and 'Buffer' not in v.name]
